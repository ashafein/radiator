...
class ControllerModuleHomepage extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
			$this->model_fido_homepage->addHomepage($this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->redirect($this->url->link('module/homepage/listing', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	private function getModule() {
		...
		$this->data['breadcrumbs'][] = array(
     		...
     		'href'      => $this->url->link('module/homepage', 'token=' . $this->session->data['token'], 'SSL'),
     		'separator' => ' :: '
  		);

		$this->data['homepages'] = $this->url->link('module/homepage/listing', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['action'] = $this->url->link('module/homepage', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['homepage_layout_option'])) {
			...
			$this->data['homepage_layout_option'] = $this->config->get('homepage_layout_option');
		}

		if (isset($this->request->post['homepage_grid_cols'])) {
			$this->data['homepage_grid_cols'] = $this->request->post['homepage_grid_cols'];
		} else {
			$this->data['homepage_grid_cols'] = $this->config->get('homepage_grid_cols');
		}
		...
		} elseif ($this->config->get('homepage_module')) { 
			$this->data['modules'] = $this->config->get('homepage_module');
		}				
	...
	private function getList() {
		...
		if (isset($this->session->data['success'])) {
  		...
  		$this->data['breadcrumbs'][] = array(
     		'text'      => $this->language->get('heading_title'),
     		'href'      => $this->url->link('module/homepage/listing', 'token=' . $this->session->data['token'], 'SSL'),
     		'separator' => ' :: '
  		);

		$this->data['module'] = $this->url->link('module/homepage', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['insert'] = $this->url->link('module/homepage/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('module/homepage/delete', 'token=' . $this->session->data['token'], 'SSL');
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('module/homepage/update', 'token=' . $this->session->data['token'] . '&homepage_id=' . $result['homepage_id'], 'SSL')
			);
	...
	private function getForm() {
		...
		if (!isset($this->request->get['homepage_id'])) {
			$this->data['action'] = $this->url->link('module/homepage/insert',  'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['action'] = $this->url->link('module/homepage/update', 'token=' . $this->session->data['token'] . '&homepage_id=' . $this->request->get['homepage_id'], 'SSL');
		}

		$this->data['cancel'] = $this->url->link('module/homepage/listing', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($homepage_info) && $homepage_info['image'] && file_exists(DIR_IMAGE . $homepage_info['image'])) {
			$this->data['preview'] = $this->model_tool_image->resize($homepage_info['image'], 100, 100);
		} else {
